...
## Mission
Provide command-line utilities (`dtnprint`, `dtnsend`, `dtnquery`, `dtntrigger`, and `dtnping`) for interacting with the `hardy` BPA implementation of DTN BPv7.
...
## Directory Map
- `src/bin/`: Main executable utilities (`dtnprint`, `dtnsend`, `dtnquery`, `dtntrigger`, `dtnping`).
